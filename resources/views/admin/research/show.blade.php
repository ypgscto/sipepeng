<x-app-layout>
    <x-slot name="header">Detail Proposal Penelitian</x-slot>

    <div class="sipeng-page space-y-6">
        <x-sipeng.page-header :title="$proposal->judul" :description="$proposal->proposal_number">
            <x-slot name="actions">
                <a href="{{ route('admin.research.index') }}" class="sipeng-btn-secondary text-sm">Daftar</a>
                @if ($canEdit)
                    <a href="{{ route('admin.research.edit', $proposal) }}" class="sipeng-btn-primary text-sm">Edit</a>
                @endif
                @if ($canSubmit)
                    <form method="POST" action="{{ route('admin.research.submit', $proposal) }}" onsubmit="return confirm('Ajukan proposal ini?')">
                        @csrf
        ...
        <div class="sipeng-card">
            <div class="sipeng-card-body">
                <div class="flex flex-wrap items-center gap-3 mb-4">
                    @include('admin.research.partials.status-badge', ['proposal' => $proposal])
                ...
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    ...
                    <div><dt class="text-slate-500">Bidang Fokus</dt><dd>{{ $proposal->bidangFokus?->name ?? '—' }}</dd></div>
                    <div><dt class="text-slate-500">Rumpun Ilmu</dt><dd>{{ $proposal->rumpunIlmu?->name ?? '—' }}</dd></div>
                    <div><dt class="text-slate-500">Jadwal</dt><dd>{{ $proposal->jadwal_mulai?->format('d M Y') ?? '—' }} — {{ $proposal->jadwal_selesai?->format('d M Y') ?? '—' }}</dd></div>
                    <div><dt class="text-slate-500">Lokasi</dt><dd>{{ $proposal->lokasi ?? '—' }}</dd></div>
                </dl>

                @foreach (['ringkasan','latar_belakang','rumusan_masalah','tujuan','manfaat','metode','target_luaran'] as $field)
                    @if ($proposal->{$field})
                        <div class="mt-4">
                            <h4 class="text-sm font-semibold text-slate-700 capitalize">{{ str_replace('_', ' ', $field) }}</h4>
        ...
        <div class="sipeng-card">
            <div class="sipeng-card-body">
                ...
                <ul class="space-y-2 text-sm">
                    @foreach (['file_proposal' => 'Proposal', 'file_pengesahan' => 'Pengesahan', 'file_pernyataan' => 'Pernyataan'] as $field => $label)
                        <li class="flex items-center justify-between">
                            <span>{{ $label }}</span>
                            @if ($proposal->{$field})
                                <a href="{{ route('admin.research.download', [$proposal, $field]) }}" class="text-emerald-700 hover:underline">{{ $proposal->{$field.'_name'} }}</a>
                            @else
        ...
        @if ($canVerifyAdmin)
            <div class="sipeng-card border-indigo-200">
                <div class="sipeng-card-body">
                    ...
                    <form method="POST" action="{{ route('admin.research.admin-verification.store', $proposal) }}" class="space-y-3">
                        @csrf
        ...
        @if ($canAssignReviewer)
            <div class="sipeng-card">
                <div class="sipeng-card-body">
                    ...
                    <form method="POST" action="{{ route('admin.research.review.assign', $proposal) }}" class="flex flex-wrap gap-3 items-end">
                        @csrf
        ...
        @if ($canSubmitReview && $myReview)
            <div class="sipeng-card border-violet-200">
                <div class="sipeng-card-body">
                    ...
                    <form method="POST" action="{{ route('admin.research.review.submit', $proposal) }}" class="space-y-3">
                        @csrf
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
        ...
        @if ($canDecide)
            <div class="sipeng-card border-emerald-200">
                <div class="sipeng-card-body">
                    <h3 class="font-semibold text-slate-900 mb-3">Penetapan Penelitian</h3>
                    <form method="POST" action="{{ route('admin.research.decision.store', $proposal) }}" class="space-y-3">
                        @csrf
                        <div>
                            <label class="sipeng-label">Keputusan</label>
                            <select name="decision" class="sipeng-input" required>
        ...
        <div class="sipeng-card">
            <div class="sipeng-card-body">
                <h3 class="font-semibold text-slate-900 mb-3">Etik Penelitian & Luaran</h3>
                <div class="grid sm:grid-cols-2 gap-4 text-sm">
                    <div>
                        <h4 class="font-medium mb-2">Etik</h4>
                        @if ($proposal->ethicsApplications->isEmpty())
                            <p class="text-slate-500">Belum ada aplikasi etik.</p>
                            @if ($canEdit ?? false)
                                <a href="{{ route('admin.research-ethics.create', ['research_proposal_id' => $proposal->id]) }}" class="text-emerald-700 text-xs hover:underline">Ajukan etik</a>
                            @endif
                        @else
                            <ul class="space-y-1">@foreach($proposal->ethicsApplications as $ethics)<li><a href="{{ route('admin.research-ethics.show', $ethics) }}" class="text-emerald-700 hover:underline">{{ $ethics->application_number }}</a> — @include('admin.research-ethics.partials.status-badge', ['record'=>$ethics])</li>@endforeach</ul>
                        @endif
                    </div>
                    <div>
                        <h4 class="font-medium mb-2">Publikasi & HKI terkait</h4>
                        <ul class="space-y-1">
                            @forelse($proposal->publications as $pub)<li><a href="{{ route('admin.publications.show', $pub) }}" class="text-emerald-700 hover:underline">{{ $pub->registration_number }}</a></li>@empty @endforelse
                            @forelse($proposal->ipRegistrations as $ip)<li><a href="{{ route('admin.hki.show', $ip) }}" class="text-emerald-700 hover:underline">{{ $ip->registration_number }}</a></li>@empty @endforelse
                            @if($proposal->publications->isEmpty() && $proposal->ipRegistrations->isEmpty())<li class="text-slate-500">Belum ada luaran.</li>@endif
                        </ul>
                        <div class="mt-2 flex gap-2 text-xs">
                            <a href="{{ route('admin.publications.create', ['research_proposal_id'=>$proposal->id]) }}" class="text-emerald-700 hover:underline">+ Publikasi</a>
                            <a href="{{ route('admin.hki.create', ['research_proposal_id'=>$proposal->id]) }}" class="text-emerald-700 hover:underline">+ HKI</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="sipeng-card">
            <div class="sipeng-card-body">
                ...
                <div class="flex flex-wrap gap-2 text-xs">
                    <a href="{{ route('admin.letters.create.from-research', [$proposal, 'surat_tugas_penelitian']) }}" class="text-emerald-700 hover:underline">+ ST Penelitian</a>
                    <a href="{{ route('admin.letters.create.from-research', [$proposal, 'surat_izin_penelitian']) }}" class="text-emerald-700 hover:underline">+ Izin Penelitian</a>
                    <a href="{{ route('admin.letters.create.from-research', [$proposal, 'surat_undangan_reviewer']) }}" class="text-emerald-700 hover:underline">+ Undangan Reviewer</a>
                </div>
            </div>
        </div>
