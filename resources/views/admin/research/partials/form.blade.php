...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
        ...
        <select name="tahun_akademik_id" id="tahun_akademik_id" class="sipeng-input" required
            onchange="syncTahunAkademikLabel(this)">
    ...
    <div>
        <label class="sipeng-label">Skema Penelitian <span class="text-red-600">*</span></label>
        ...
        @if ($schemes->isEmpty())
            <p class="mt-1 text-xs text-amber-700">
                Belum ada skema penelitian.
                <a href="{{ route('admin.master.research-schemes.create') }}" class="font-semibold underline hover:text-amber-900">Tambah skema</a>
            </p>
        @endif
    </div>
</div>
...
<div class="mt-4">
    <label class="sipeng-label">Judul Penelitian <span class="text-red-600">*</span></label>
    <input type="text" name="judul" value="{{ old('judul', $proposal?->judul) }}" class="sipeng-input" required>
</div>

<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
    <div>
        <label class="sipeng-label">Ketua Peneliti (SIAKAD ID)</label>
    ...
    <div>
        <label class="sipeng-label">Bidang Fokus</label>
        <select name="bidang_fokus_id" class="sipeng-input">
            <option value="">— Pilih —</option>
            @foreach ($focusAreas as $item)
                <option value="{{ $item->id }}" @selected((string) old('bidang_fokus_id', $proposal?->bidang_fokus_id) === (string) $item->id)>{{ $item->name }}</option>
            @endforeach
        </select>
    </div>
    <div>
        <label class="sipeng-label">Rumpun Ilmu</label>
        <select name="rumpun_ilmu_id" class="sipeng-input">
            <option value="">— Pilih —</option>
            @foreach ($scienceClusters as $item)
                <option value="{{ $item->id }}" @selected((string) old('rumpun_ilmu_id', $proposal?->rumpun_ilmu_id) === (string) $item->id)>{{ $item->name }}</option>
            @endforeach
        </select>
    </div>
</div>
...
<div class="mt-4 space-y-4">
    @foreach (['ringkasan' => 'Ringkasan', 'latar_belakang' => 'Latar Belakang', 'rumusan_masalah' => 'Rumusan Masalah', 'tujuan' => 'Tujuan', 'manfaat' => 'Manfaat', 'metode' => 'Metode', 'target_luaran' => 'Target Luaran'] as $field => $label)
        <div>
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
    <div>
        <label class="sipeng-label">Lokasi</label>
        <input type="text" name="lokasi" value="{{ old('lokasi', $proposal?->lokasi) }}" class="sipeng-input">
    </div>
    <div>
...
<div class="mt-6">
    <h3 class="font-semibold text-slate-900 mb-3">RAB Penelitian</h3>
...
<div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
    @foreach (['file_proposal' => 'File Proposal (PDF)', 'file_pengesahan' => 'File Pengesahan (PDF)', 'file_pernyataan' => 'File Pernyataan (PDF)'] as $field => $label)
        <div>
...
<p class="mt-2 text-xs text-slate-500">
    Maks. unggahan: proposal {{ (int) (config('sipepeng_research.uploads.file_proposal.max_kb') / 1024) }} MB,
    pengesahan & pernyataan {{ (int) (config('sipepeng_research.uploads.file_pengesahan.max_kb') / 1024) }} MB per berkas (PDF).
    Jika muncul error sesi, perkecil ukuran berkas atau muat ulang halaman sebelum mengirim.
</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ['tahun_akademik_id','semester_id','prodi_id'].forEach(id => {
